...
import {
  AllAvailableUsers,
  Users,
} from '../../types/interfaces/Student/EmploymentInterface';
...
export const HrAvailableStudents = () => {
  const [allAvailableUsers, setAllAvailableUsers] = useState<Users | null>(
    null
  );
  const itemsOnPage = useSelector(
    (state: RootState) => state.pagination.itemsOnPage
  );
  ...
  return (
    <>
      ...
      <WrapperHr>
        ...
        <UtilsHr />
        <AvailableStudents
          users={allAvailableUsers?.users as AllAvailableUsers[]}
        />
      </WrapperHr>
      <Pagination />
    </>
